...
import { FormConfig, SocraticQuestionConfig } from "@/lib/api";
...
const DEFAULT_SOCRATIC_DESCRIPTION = `你是一位运用苏格拉底式提问法的导师。你的任务是通过提问引导学生深入思考，而非直接给出答案。

【教学原则】
1. 通过追问帮助学生发现知识漏洞
2. 用启发性问题引导学生自己得出结论
3. 表扬学生的思考过程，而非仅仅是正确答案
4. 当学生偏离方向时，温和地引导回正轨
5. 鼓励学生用自己的话解释概念
...
export default function FormEditor({ form, onUpdate, onDelete }: FormEditorProps) {
  ...
  const handleTypeChange = (newType: "traditional" | "socratic") => {
    const updated = {
      ...localForm,
      type: newType,
      // Initialize Socratic fields if switching to Socratic mode
      ...(newType === "socratic" && !localForm.teaching_content ? {
        teaching_content: "",
        questions: localForm.questions || [],
        description: localForm.description || DEFAULT_SOCRATIC_DESCRIPTION,
        fields: [], // Socratic mode doesn't use fields
      } : {}),
      // Reset to traditional fields if switching back
      ...(newType === "traditional" ? {
        fields: localForm.fields.length > 0 ? localForm.fields : ["字段1"],
      } : {})
    };
    setLocalForm(updated);
    onUpdate(updated);
  ...
  // Traditional mode: field management
  const addField = () => {
    if (newField.trim()) {
  ...
  // Socratic mode: question management
  const addQuestion = () => {
  ...
  const isSocratic = localForm.type === "socratic";

  return (
    <div className="space-y-6">
      {/* Form Type Selector */}
      <div className="p-4 bg-gradient-to-r from-[#0071e3]/5 to-[#34c759]/5 rounded-xl border border-[#d2d2d7]/50">
        <label className="block text-xs font-medium text-[#86868b] mb-3">表单类型</label>
        <div className="flex gap-3">
          ...
          >
            <div className={`text-[15px] font-semibold mb-1 ${!isSocratic ? "text-[#0071e3]" : "text-[#1d1d1f]"}`}>
              传统模式
            </div>
          ...
          <motion.button
            onClick={() => handleTypeChange("socratic")}
            whileTap={{ scale: 0.98 }}
            className={`flex-1 p-4 rounded-xl border-2 transition-all ${
              isSocratic
                ? "border-[#34c759] bg-[#34c759]/5"
                : "border-[#d2d2d7]/50 hover:border-[#34c759]/30"
            }`}
          >
            <div className={`text-[15px] font-semibold mb-1 ${isSocratic ? "text-[#34c759]" : "text-[#1d1d1f]"}`}>
              苏格拉底模式
            </div>
            <div className="text-xs text-[#86868b]">
              问答引导模式，用于形成性教学
      ...
      <div className="space-y-4">
        ...
        <div>
          <label className="block text-xs font-medium text-[#86868b] mb-2">
            AI提示词 {isSocratic && <span className="text-[#34c759]">（苏格拉底对话系统提示）</span>}
          </label>
          <textarea
      ...
      {/* Socratic Mode: Teaching Content & Questions */}
      {isSocratic && (
        <div className="space-y-6">
          {/* Teaching Content */}
          <div className="p-4 bg-[#34c759]/5 rounded-xl border border-[#34c759]/20">
            <label className="block text-xs font-medium text-[#34c759] mb-2">
              教学内容（知识点）
            </label>
            <p className="text-xs text-[#86868b] mb-3">
              输入您希望学生掌握的知识内容，系统将基于此内容进行苏格拉底式引导教学。
            </p>
            <textarea
              value={localForm.teaching_content || ""}
              onChange={(e) => handleChange("teaching_content", e.target.value)}
              rows={6}
              placeholder="请输入教学内容，例如：牛顿第一定律指出，物体在没有受到外力作用时，将保持静止状态或匀速直线运动状态..."
              className="w-full px-4 py-3 bg-white rounded-xl text-[15px] text-[#1d1d1f] placeholder-[#86868b] outline-none focus:ring-2 focus:ring-[#34c759] transition-all resize-none"
            />
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <label className="block text-xs font-medium text-[#86868b]">
                引导问题列表 <span className="text-[#34c759]">({(localForm.questions || []).length} 个问题)</span>
              </label>
              <motion.button
                onClick={addQuestion}
                ...
                {(localForm.questions || []).map((question, index) => (
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <span className="text-xs font-semibold text-[#34c759] bg-[#34c759]/10 px-2 py-1 rounded-lg">
                        问题 {index + 1}
                      </span>
                      <div className="flex items-center gap-1">
                        <button
                          ...
                          disabled={index === 0}
                          className="w-6 h-6 flex items-center justify-center text-[#86868b] hover:text-[#1d1d1f] disabled:opacity-30 transition-colors"
                        >
                          ↑
                        </button>
                        <button
                        ...
                        <button
                          ...
                          className="w-6 h-6 flex items-center justify-center text-[#ff3b30] hover:bg-[#ff3b30]/10 rounded-lg transition-colors"
                        >
                          ×
                        </button>
                      </div>
                    ...
                    <div className="space-y-3">
                      <div>
                        <label className="block text-xs text-[#86868b] mb-1">问题内容</label>
                        <textarea
            ...
            {(localForm.questions || []).length === 0 && (
              <div className="text-center py-8 text-[#86868b]">
                ...
                <p className="text-sm">暂无问题，点击上方按钮添加</p>
              </div>
            )}
          </div>
        </div>
      ...
      {/* Extraction Prompt Section (only for traditional mode) */}
      {!isSocratic && (
      ...
      {/* Prompt Preview Section (only for traditional mode) */}
      {!isSocratic && <PromptPreview form={localForm} />}
